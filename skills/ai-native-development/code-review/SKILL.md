...
Verify that the runtime can create isolated review contexts. When both axes are active, dispatch the independent review sub-agents concurrently through the runtime's available delegation capability. If that capability is unavailable, state the precise capability gap and stop. Do not fall back to sequential review.
