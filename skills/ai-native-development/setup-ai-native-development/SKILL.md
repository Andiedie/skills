...
3. Decide the workflow state backend.
...
4. Decide the GitHub issue surface when the backend is `github-native`.
...
5. Decide stage state and lifecycle rules.
...
6. Decide relationships and claims.
...
7. Decide domain, decision, and rejection sources.
...
8. Draft the changes for review.
...
10. Report.
   - List files changed, backend choice, `.and/config.yml`, labels created or mapped, backend assumptions, PR-as-request setting when relevant, relationship rules, claim rules, implementation isolation rules, domain and ADR locations, out-of-scope convention, and skills now ready to use.
