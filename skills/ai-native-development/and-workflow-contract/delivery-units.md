...
The latest `## Implementation` receipt is the sole deployment-handoff candidate; an incomplete or mismatched latest receipt never falls back to an older one. A newer implementation head requires a new reviewed receipt and disposition, plus a new Manifest when the disposition is `custom`; deployment evidence does not replace or mutate either artifact.
