...
| Implement | The current actor owns or is delegated a claimed delivery unit whose implementation, review, Deployment disposition, or Cleanup disposition is incomplete. | [`and-implement`](../and-implement/SKILL.md) | Committed implementation, verification, review, and reviewed-head-bound Deployment and Cleanup dispositions, with a Deployment Manifest only for `custom` and typed cleanup items only for `required`. | `and-finish`, an acceptance owner, or a route back |
| Finish | Reviewed delivery has no pending acceptance or blocker, or an earlier finish needs to resume delivery, lifecycle completion, or terminal cleanup. | [`and-finish`](../and-finish/SKILL.md) | Merged pull request, linked handoffs, terminal lifecycle evidence, verified typed local cleanup, and safe Git artifact cleanup. | A new follow-up signal, environment deployment, or done |
| Audit | Stage state, State Reasons, relationships, blockers, ownership, implementation artifacts, Deployment and Cleanup dispositions, or lifecycle outcomes may have drifted. | [`and-sweep`](../and-sweep/SKILL.md) | Actionable findings and approved low-risk cleanup. | The skill or owner responsible for the repair |
