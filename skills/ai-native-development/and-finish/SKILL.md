...
1. **Resolve the transaction.**
   - Read the complete delivery unit, claim, contract, every PRD child, latest Implementation receipt and any embedded review attestation, Deployment disposition and any present Cleanup handoff, the conditional Deployment Manifest or typed cleanup items when required, verification, review, and—when declared—the Acceptance Gate and latest receipt from [acceptance-gate.md](../and-workflow-contract/acceptance-gate.md), plus blockers, relationships, and linked artifacts.
...
5. **Revalidate the final head.**
   ...
   - For every stable CI check or workflow identity selected by the Package Verification Plan, verify that configured identity against the final pull-request head before merge.
   - The shadow result is visible evidence only. Its status alone does not block, authorize, or reroute Finish or change existing review, acceptance, deployment, cleanup, or lifecycle authority. Prove required checks, reviews, and blocker evidence apply to that head and are successful; preserve the independent shared Acceptance result. No blocking review or conflict remains, and GitHub reports it mergeable.
