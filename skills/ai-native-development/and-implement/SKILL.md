...
Route an unready or unowned unit to the smallest upstream AND skill. If the current head already has clean implementation evidence, an authoritative Deployment disposition, a valid optional Cleanup handoff when resources survive, a complete Deployment Manifest when Deployment is `custom`, and every conditional authority reports no remaining Implement action, route to `and-finish`. If implementation and review are clean but Deployment is missing or stale, or an emitted Cleanup handoff is malformed or stale, resume at handoff classification; route a linked non-authoritative finish proposal to `and-finish` before implementation resumes.
...
1. **Resolve the work.**
   ...
   - When the contract declares a gate, use [acceptance-gate.md](../and-workflow-contract/acceptance-gate.md) to read it and retain only the inputs requested for an Implement action.
...
3. **Implement the contract.**
   ...
   - Consume the Package Contract's record-only Verification Plan. Complete every selected local evidence item before publishing the Implementation receipt. If implementation reveals additional risk, strengthen the plan's evidence and record actual evidence plus material deviations in the existing `Verification` section. Omit or substitute planned evidence only with a recorded reason and equivalent risk or failure coverage; otherwise route the uncovered decision to `and-pack`. Time cost or a failing check does not authorize reduced coverage. A proven baseline flake permits an equivalent substitution, not a waiver. Do not add a receipt field for this plan.
   ...
   - Perform only the Implement action returned by the shared Acceptance authority; keep Cleanup and Deployment work with their existing handoff owners.
   ...
   - Completion criterion: code, tests, and required docs satisfy every parent and child requirement, with implementation evidence and handoff classifications recorded, or one evidenced upstream blocker is named.
...
4. **Review the complete diff.**
   ...
   - Keep the committed candidate unchanged until `code-review` returns the complete Standards and Spec reports. Preserve their axis distinction and handle all findings from that cycle as one batch: fix every in-scope implementation finding, route contract defects to `and-pack`, and route human-owned judgments through the current State Reason owner or `and-triage` when no decision route exists. Spec review checks each documentation item against its source, Package entry, and diff.
   - Rerun relevant verification and commit the updated candidate before invoking the repository-owned `code-review` again over the complete diff.
...
5. **Classify deployment and local cleanup handoffs.**
   ...
   - When the shared Acceptance authority routes a side-effect refresh here, prepare a superseding Implementation receipt that links the Acceptance permalink and the refreshed Cleanup or Deployment handoff.
...
6. **Record the handoff.**
   ...
   - Only after the current Implementation receipt is written and read back, perform any owner handoff returned by the shared Acceptance authority with the exact reviewed binding, current PR/head, evidence expectation, and public delivery Issue receipt location.
